...
def recalculo_rateio(matricula):
    ...
    script_sql_rateio = f"""--SDES-{seq_imp} || Recalculo de Rateio
...
def update_consumo(matricula):
    ...
    colunas_disponiveis = ['confat', 'medconmed', 'media_consumo_faturado', 'credito_consumo', 'leitura', 'credito_utilizado', 'conmed', 'id_conf']
...
def gerar_conta(matricula):
    ...
    script_sql_conta = f"""--SDES-{seq_imp} ||  CANCELA E INSERE CONTA, PARCELA E RETIFICACAO POS RECALCULO DE RATEIO REF. {referencia}
...
seq_imp = input("Digite o número do chamado: ")  # Adicionando o número do chamado
limpar_tela()
